...
from data import create_carla_dataset, create_carla_loader
from model.img2map import img2map_model
...
def main():
    ...
    #     backbone_linear_scaled_lr = args.backbone_lr * args.batch_size / 512.0
    #     backbone_weights = []
    #     other_weights = []
    ...
    if args.rank == 0:
        ...
        output_dir = get_outdir(args.output if args.output else "./logs", exp_name)
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            ...
            loss = loss_traffic * 0.5 + loss_velocity * 0.05
        ...
        if last_batch or batch_idx % args.log_interval == 0:
            ...
            if writer and args.local_rank == 0:
                writer.add_scalar("train/loss", losses_m.avg, num_updates)
                writer.add_scalar("train/loss_traffic", losses_traffic.avg, num_updates)
                writer.add_scalar("train/loss_velocity", losses_velocity.avg, num_updates)
                ...
                writer.add_image(
                    ...
                    torch.clip(
                        torch.tensor(
                            render(output[0][0].view(20, 20, 7).detach().cpu().numpy())[
                                :200, 80:280
                            ]
                        ),
                        0,
        ...
        for batch_idx, (input, target) in enumerate(loader):
            ...
            if args.local_rank == 0 and (last_batch or batch_idx % args.log_interval == 0):
                ...
                _logger.info(
                    ...
                    "Loss(traffic): {loss_traffic.val:>7.4f} ({loss_traffic.avg:>6.4f}) ".format(
                        log_name,
                        batch_idx,
                        last_idx,
                        batch_time=batch_time_m,
                ...
                if writer:
                    # Add Image
                    input["lidar"][0] = input["lidar"][0] / torch.max(input["lidar"][0])
                    writer.add_image("val/lidar", torch.clip(input["lidar"][0], 0, 1), epoch)
                    writer.add_image(
                        "val/front_view",
