...
def generate_det_data(heatmap, measurements, actors_data, pixels_per_meter=5, max_distance=18):
    traffic_heatmap = block_reduce(
        heatmap, block_size=(pixels_per_meter, pixels_per_meter), func=np.mean
    )
    ...
    for _id in actors_data:
        ...
        if dis <= 1 or dis >= (max_distance + 3) ** 2 * 2 or "box" not in actors_data[_id]:
