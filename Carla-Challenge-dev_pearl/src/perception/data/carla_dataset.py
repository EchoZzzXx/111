...
class CarlaMVDetDataset(BaseIODataset):
    ...
    def __getitem__(self, idx):
        ...
        rgb_image = self._load_image(os.path.join(route_dir, "rgb_front", "%04d.jpg" % frame_id))
        ...
        if self.with_waypoints:
            waypoints_data = self._load_npy(os.path.join(route_dir, "waypoints.npy"))[frame_id]
        ...
        if self.with_lidar:
            ...
            lidar_processed = lidar_to_histogram_features(full_lidar, crop=self.input_lidar_size)
        ...
        if any(np.isnan(local_command_point)):
            local_command_point[np.isnan(local_command_point)] = np.mean(local_command_point)
        ...
        if self.head == "det":
            heatmap = generate_heatmap(
                copy.deepcopy(measurements), copy.deepcopy(actors_data), pixels_per_meter=10
            )
            det_data = (
                generate_det_data(
                    heatmap,
                    copy.deepcopy(measurements),
                    copy.deepcopy(actors_data),
                    pixels_per_meter=10,
                )
                .reshape(400, -1)
                .astype(np.float32)
            ...
            img_traffic = generate_heatmap(copy.deepcopy(measurements), copy.deepcopy(actors_data))
