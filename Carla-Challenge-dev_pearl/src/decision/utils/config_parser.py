...
class AgentConfig:
    ...
    @classmethod
    def set_scenario(cls, scenario_name):
        if cls.exp_config_dict['override_name'] is not None:
            cls.scenario_name = cls.exp_config_dict['override_name']
        else:
            cls.scenario_name = scenario_name

        mask_scenario_map = {
            0: ["ParkingExit", "Accident", "ConstructionObstacle", "OppositeVehicleRunningRedLight", "HazardAtSideLane"],
            1: ["SignalizedJunctionLeftTurn", "SignalizedJunctionRightTurn",
                "OppositeVehicleRunningRedLight", "NonSignalizedJunctionLeftTurn",
                "PriorityAtJunction", "OppositeVehicleRunningRedLight","JunctionTurn"],
            2: ["StraightForward", "VehicleTurningRoute", "EnterActorFlowV2", "StaticCutIn", "DynamicObjectCrossing"],
            3: ["ParkedObstacleTwoWays", "ConstructionObstacleTwoWays", "AccidentTwoWays"],
        }

        for mask_id, scenarios in mask_scenario_map.items():
            if cls.scenario_name in scenarios:
                cls.action_mask_type = mask_id

        assert cls.action_mask_type is not None
