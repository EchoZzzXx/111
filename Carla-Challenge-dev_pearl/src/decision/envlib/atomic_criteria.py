...
class DrivenDistanceTest(Criterion):
    ...
    def __init__(
        self, actor, distance, acceptable_distance=None, optional=False, name="CheckDrivenDistance"
    ):
        """
        Setup actor
        """
...
class AverageVelocityTest(Criterion):
    ...
    def __init__(
        self, actor, velocity, acceptable_velocity=None, optional=False, name="CheckAverageVelocity"
    ):
        """
...
class CollisionTest(Criterion):
    ...
    def update(self):
        ...
        self.actor_velocity = CarlaDataProvider.get_velocity(self.actor)
            ...
            if linear_speed is not None:
                if linear_speed < self._min_speed and self._time_last_valid_state:
                    if (GameTime.get_time() - self._time_last_valid_state) > self._max_time:
                        ...
                        event = TrafficEvent(
                            event_type=TrafficEventType.VEHICLE_BLOCKED, frame=GameTime.get_frame()
                        )
                        event.set_message(
...
class OffRoadTest(Criterion):
    ...
    def __init__(
        self, actor, duration=0, optional=False, terminate_on_failure=False, name="OffRoadTest"
    ):
        """
...
class EndofRoadTest(Criterion):
    ...
    def __init__(
        self, actor, duration=0, optional=False, terminate_on_failure=False, name="EndofRoadTest"
    ):
        """
        Setup of the variables
        """
...
class OnSidewalkTest(Criterion):
    ...
    def __init__(
        self, actor, duration=0, optional=False, terminate_on_failure=False, name="OnSidewalkTest"
    ):
        """
        Construction with sensor setup
        """
...
class OutsideRouteLanesTest(Criterion):
    ...
    def __init__(self, actor, route, terminate_on_failure, optional=False, name="OutsideRouteLanesTest"):
        """
        Constructor
        """
    ...
    def update(self):
        ...
        # calculate distance between ego point and the route line
        # proj = wp_veh.dot(wp_dir) / wp_dir.squared_length()
        # self.distance = (wp_veh - proj * wp_dir).length()
        # print('TIPS: showing the projected distance: ', self.distance)
    ...
    def _is_outside_driving_lanes(self, location):
        ...
        self.distance = driving_distance
    ...
    def _is_at_wrong_lane(self, location):
        ...
        # print('showing lane id of vehicle:', lane_id)
        ...
        if angle < 180:
            ...
            self.angle = (360 - angle) * np.pi / 180
...
class WrongLaneTest(Criterion):
    ...
    def update(self):
        ...
        if (
            ...
            if self._previous_lane_waypoint.is_junction:
                ...
                vehicle_lane_angle = math.degrees(
                    math.acos(
                        np.clip(
                            np.dot(vector_actor, vector_wp) / (np.linalg.norm(vector_wp)), -1.0, 1.0
                        )
                    )
                )
        ...
        if self._in_lane and self._wrong_distance > 0:
            wrong_way_event = TrafficEvent(
                event_type=TrafficEventType.WRONG_WAY_INFRACTION, frame=GameTime.get_frame()
            )
            self._set_event_message(
                wrong_way_event,
    ...
    def _set_event_dict(self, event, location, distance, road_id, lane_id):
        ...
        """
        event.set_dict(
            {"location": location, "distance": distance, "road_id": road_id, "lane_id": lane_id}
        )
    ...
    def update(self):
        ...
        if self._terminate_on_failure and (self.test_status == "FAILURE"):
            ...
            for index in range(
                ...
                distance = math.sqrt(
                    ((location.x - ref_location.x) ** 2) + ((location.y - ref_location.y) ** 2)
                )
            ...
            if self._current_index != closest_index:
                new_dist = (
                    self._accum_meters[closest_index] - self._accum_meters[self._current_index]
                )
...
class RouteCompletionTest(Criterion):
    ...
    def __init__(self, actor, route, name="RouteCompletionTest", terminate_on_failure=False):
        ...
        self.meter_diff = self.actual_meter = self.last_meter = 0.0
        ...
        self.scenario_length = AgentConfig.exp_config_dict['single_scence_length']
        self.is_junction_scenario = (AgentConfig.action_mask_type == 1)
        self.junction_end_road_id = AgentConfig.exp_config_dict['junction_end_id']
        self.step = 0
    ...
    def update(self):
        ...
        if self._terminate_on_failure and (self.test_status == "FAILURE"):
            ...
            for index in range(
                ...
                if wp_veh.dot(wp_dir) > 0:
                    self._index = index
                    self.actual_meter = self.accum_meters[self._index] + wp_veh.dot(wp_dir)
                    self.actual_value = self._route_accum_perc[self._index]
                    forward = True

            # check if vehicle move backward
            if not forward:
                for index in range(self._index, max(self._index - self.WINDOWS_SIZE - 1, 0), -1):
                    # Get the dot product to know if it has passed this location
                    route_transform = self._route_transforms[index]
                    route_location = route_transform.location
                    wp_dir = route_transform.get_forward_vector()  # Waypoint's forward vector
            ...
            self.meter_diff = self.actual_meter - self.last_meter
            if self.actual_meter - self.last_meter >= 1:
                self.route_tick = True
                self.last_meter = self.actual_meter
            else:
                self.route_tick = False
            # self.last_meter = self.actual_meter
            ...
            road_id = waypoint.road_id
            # print('>')     
            if self.is_junction_scenario:
                if road_id == self.junction_end_road_id:
                    self.test_status = "FAILURE"
            else:
                if (self.actual_meter >= self.scenario_length):
                    self.test_status = "FAILURE"
...
class RunningStopTest(Criterion):
    ...
    def point_inside_boundingbox(self, point, bb_center, bb_extent, multiplier=1.2):
        ...
        A = carla.Vector2D(
            bb_center.x - multiplier * bb_extent.x, bb_center.y - multiplier * bb_extent.y
        )
        B = carla.Vector2D(
            bb_center.x + multiplier * bb_extent.x, bb_center.y - multiplier * bb_extent.y
        )
...
class MinimumSpeedRouteTest(Criterion):
    ...
    def __init__(
        self, actor, route, checkpoints=1, name="MinimumSpeedRouteTest", terminate_on_failure=False
    ):
        """ """
        ...
        self.steer_diff = 0
        self.throttle_diff = 0
        self._last_throttle = 0
        self._last_steer = 0

    def check_reward(self):
        self.actor_steer = self.actor.get_control().steer
        self.actor_throttle = self.actor.get_control().throttle
        self.actor_brake = self.actor.get_control().brake

        self.steer_diff = self.actor_steer - self._last_steer
        self._last_steer = self.actor_steer

        self.throttle_diff = self.actor_throttle - self._last_throttle
        self._last_throttle = self.actor_throttle

        self.speed_limit = self.actor.get_speed_limit()
        self.red_light_state = self.actor.get_traffic_light_state() == carla.TrafficLightState.Red
        self.actor_velocity = CarlaDataProvider.get_velocity(self.actor)
    ...
    def update(self):
        ...
        if background_vehicles:
            for vehicle in background_vehicles:
                self.frame_mean_speed += CarlaDataProvider.get_velocity(vehicle)
            self.frame_mean_speed /= len(background_vehicles)

            # Record the data
            self._mean_speed += self.frame_mean_speed
